...
CONTAINS
  ...
  real(crm_rknd) function term_vel_qp(ncrms,icrm,i,j,k,ind,micro_field,tabs,rho,a_pr,a_gr,vrain,crain,vsnow,csnow,vgrau,cgrau)
    use vars
    !$acc routine seq
    implicit none
    ...
    real(crm_rknd), intent(in) :: tabs(nx,ny,nzm,ncrms), rho(nzm,ncrms)
    real(crm_rknd), intent(in) :: a_pr, a_gr, vrain, crain, csnow, vsnow, cgrau, vgrau
  ...
  subroutine precip_fall(ncrms,micro_field, hydro_type, omega, ind)
    ...
    prec_cfl = 0.
    !$acc parallel loop collapse(4) copyin(micro_field,tabs,rho,rhofac,iwmax,rhow,dz,omega) copy(fz,www,lfac) copyout(lfac,wp,flagstat) copy(prec_cfl)
    do icrm = 1 , ncrms
      do k=1,nzm
        do j=1,ny
          do i=1,nx
            ...
            wp(i,j,k,icrm)=rhofac(k,icrm)*term_vel_qp(ncrms,icrm,i,j,k,ind,micro_field(dimx1_s,dimy1_s,1,1,1),tabs(1,1,1,1),rho(1,1),a_pr,a_gr,vrain,crain,vsnow,csnow,vgrau,cgrau)
            mytmp = wp(i,j,k,icrm)*iwmax(k,icrm)
            !$acc atomic update
            prec_cfl = max(prec_cfl,mytmp) ! Keep column maximum CFL
        ...
        do j=1,ny
          do i=1,nx
            do k=1,nzm
              ...
              mytmp = fz(i,j,k,icrm)*flagstat
              !$acc atomic update
              precflux(k,icrm) = precflux(k,icrm) - mytmp   ! For statistics
              if (k == 1) then
