...
CONTAINS
  ...
  subroutine allocate_micro()
    ...
    if (.not. allocated(fluxbmk    )) allocate( fluxbmk (nx, ny, 1:nmicro_fields) )
    if (.not. allocated(fluxtmk    )) allocate( fluxtmk (nx, ny, 1:nmicro_fields) )
    if (.not. allocated(mkwle      )) allocate( mkwle(nz,1:nmicro_fields) )
    if (.not. allocated(mkwsb      )) allocate( mkwsb(nz,1:nmicro_fields) )
    if (.not. allocated(mkadv      )) allocate( mkadv(nz,1:nmicro_fields) )
    if (.not. allocated(mklsadv    )) allocate( mklsadv(nz,1:nmicro_fields) )
    if (.not. allocated(mkdiff     )) allocate( mkdiff(nz,1:nmicro_fields) )
    if (.not. allocated(mstor      )) allocate( mstor(nz,1:nmicro_fields) )
    ...
    if (.not. associated(q )) q (dimx1_s:,dimy1_s:,1:) => micro_field(dimx1_s:dimx2_s,dimy1_s:dimy2_s,1:nzm,1)
    if (.not. associated(qp)) qp(dimx1_s:,dimy1_s:,1:) => micro_field(dimx1_s:dimx2_s,dimy1_s:dimy2_s,1:nzm,2)
