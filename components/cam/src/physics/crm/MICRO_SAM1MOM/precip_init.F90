...
contains
  ...
  subroutine precip_init(ncrms)
    ...
    !$acc parallel loop collapse(2) copyin(rho,pres) copy(tabs0,evaps2,evapr1,evapg2,accrgi,evapg1,coefice,accrgc,evaps1,evapr2,accrsi,accrrc,accrsc) async(1)
    do icrm = 1 , ncrms
      do k=1,nzm
        ...
        ! accretion by snow:
        coef1 = 0.25 * pi * nzeros * a_snow * gams1 * pratio/(pi * rhos * nzeros/rho(k,icrm) ) ** ((3+b_snow)/4.)
        ...
        evaps1(k,icrm)  =  0.65*4.*nzeros/sqrt(pi*rhos*nzeros)/(coef1+coef2)/sqrt(rho(k,icrm))
        evaps2(k,icrm)  =  0.49*4.*nzeros*gams2*sqrt(a_snow/(muelq*rrr1))/(pi*rhos*nzeros)**((5+b_snow)/8.) / (coef1+coef2) * rho(k,icrm)**((1+b_snow)/8.)*sqrt(pratio)

        ! accretion by graupel:
        coef1 = 0.25*pi*nzerog*a_grau*gamg1*pratio/(pi*rhog*nzerog/rho(k,icrm))**((3+b_grau)/4.)
        coef2 = exp(0.025*(tabs0(k,icrm) - 273.15))
        ...
        coef1  =(lsub/(tabs0(k,icrm)*rv)-1.)*lsub/(therco*rrr1*tabs0(k,icrm))
        coef2  = rv*tabs0(k,icrm)/(diffelq*rrr2*esti)
        evapg1(k,icrm)  = 0.65*4.*nzerog/sqrt(pi*rhog*nzerog)/(coef1+coef2)/sqrt(rho(k,icrm))
        evapg2(k,icrm)  = 0.49*4.*nzerog*gamg2*sqrt(a_grau/(muelq*rrr1))/(pi * rhog * nzerog)**((5+b_grau)/8.) / (coef1+coef2) * rho(k,icrm)**((1+b_grau)/8.)*sqrt(pratio)

        ! accretion by rain:
        accrrc(k,icrm)=  0.25 * pi * nzeror * a_rain * gamr1 * pratio/(pi * rhor * nzeror / rho(k,icrm)) ** ((3+b_rain)/4.)* erccoef
        ...
        evapr2(k,icrm)  =  0.31 * 2. * pi  * nzeror * gamr2 * 0.89 * sqrt(a_rain/(muelq*rrr1))/(pi * rhor * nzeror)**((5+b_rain)/8.) / (coef1+coef2) * rho(k,icrm)**((1+b_rain)/8.)*sqrt(pratio)
      end do
    end do
