  ...
  subroutine press_grad(ncrms,icrm)
    ...
    do k=1,nzm
      ...
      do j=1,ny
        ...
        do i=1,nx
          ib=i-1
          dudt(i,j,k,na)=dudt(i,j,k,na)-(p(i,j,k,icrm)-p(ib,j,k,icrm))*rdx
          dvdt(i,j,k,na)=dvdt(i,j,k,na)-(p(i,j,k,icrm)-p(i,jb,k,icrm))*rdy
          dwdt(i,j,k,na)=dwdt(i,j,k,na)-(p(i,j,k,icrm)-p(i,j,kb,icrm))*rdz
        end do ! i
      end do ! j
    end do ! k
    ...
    do k=1,nzm
      do j=1-YES3D,ny !bloss: 0,n* fixes computation of dp/d* in stats.
        do i=0,nx
          p(i,j,k,icrm)=p(i,j,k,icrm)*rho(k)  ! convert p'/rho to p'
        end do
      end do
    end do
