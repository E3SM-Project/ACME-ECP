...
contains
  ...
  subroutine pressure(ncrms)
    ...
    enddo
    !$acc parallel loop gang default(present) async(asyncid)
    do k=1,nzslab
    ...
    !$acc parallel loop collapse(3) vector_length(128) num_gangs(numgangs) private(alfa,beta) default(present) async(asyncid)
    do j=1,nyp22-jwall
      do i=1,nxp1-iwall
        do icrm = 1 , ncrms
          if(dowally) then
