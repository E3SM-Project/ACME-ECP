...
!MRN: If this is in standalone mode, lat,lon are passed in directly, not looked up in phys_grid
  ...
  !Arrays
  real(crm_rknd), pointer :: t00      (:)
  real(crm_rknd), pointer :: tln      (:)
  real(crm_rknd), pointer :: qln      (:)
  real(crm_rknd), pointer :: qccln    (:)
  real(crm_rknd), pointer :: qiiln    (:)
  real(crm_rknd), pointer :: uln      (:,:)
  real(crm_rknd), pointer :: vln      (:,:)
  real(crm_rknd), pointer :: cwp      (:,:,:)
  real(crm_rknd), pointer :: cwph     (:,:,:)
  real(crm_rknd), pointer :: cwpm     (:,:,:)
  real(crm_rknd), pointer :: cwpl     (:,:,:)
  logical       , pointer :: flag_top (:,:,:)
  real(crm_rknd), pointer :: bflx     (:)
  real(crm_rknd), pointer :: wnd      (:)
  real(crm_rknd), pointer :: colprec  (:)
  real(crm_rknd), pointer :: colprecs (:)
  integer       , pointer :: gcolindex(:)  ! array of global latitude indices
  real(crm_rknd), pointer :: cltemp   (:,:,:)
  real(crm_rknd), pointer :: cmtemp   (:,:,:)
  real(crm_rknd), pointer :: chtemp   (:,:,:)
  real(crm_rknd), pointer :: cttemp   (:,:,:)
#ifdef CLUBB_CRM
  !Array indicies for spurious RTM check
  real(kind=core_rknd), pointer :: rtm_integral_before (:,:)
  real(kind=core_rknd), pointer :: rtm_integral_after  (:,:)
  real(kind=core_rknd), pointer :: thlm_integral_before(:,:)
  real(kind=core_rknd), pointer :: thlm_integral_after (:,:)
  real(kind=core_rknd), pointer :: thlm_before         (:)
  real(kind=core_rknd), pointer :: thlm_after          (:)
  real(kind=core_rknd), pointer :: rtm_column          (:) ! Total water (vapor + liquid)     [kg/kg]
#endif
  ...
  call pool_push( t00       , (/ncrms      /) )
  call pool_push( tln       , (/plev       /) )
  call pool_push( qln       , (/plev       /) )
  call pool_push( qccln     , (/plev       /) )
  call pool_push( qiiln     , (/plev       /) )
  call pool_push( uln       , (/ncrms,plev /) )
  call pool_push( vln       , (/ncrms,plev /) )
  call pool_push( cwp       , (/ncrms,nx,ny/) )
  call pool_push( cwph      , (/ncrms,nx,ny/) )
  call pool_push( cwpm      , (/ncrms,nx,ny/) )
  call pool_push( cwpl      , (/ncrms,nx,ny/) )
  call pool_push( flag_top  , (/ncrms,nx,ny/) )
  call pool_push( bflx      , (/ncrms      /) )
  call pool_push( wnd       , (/ncrms      /) )
  call pool_push( colprec   , (/ncrms      /) )
  call pool_push( colprecs  , (/ncrms      /) )
  call pool_push( gcolindex , (/pcols      /) )
  call pool_push( cltemp    , (/ncrms,nx,ny/) )
  call pool_push( cmtemp    , (/ncrms,nx,ny/) )
  call pool_push( chtemp    , (/ncrms,nx,ny/) )
  call pool_push( cttemp    , (/ncrms,nx,ny/) )
#ifdef CLUBB_CRM
  call pool_push( rtm_integral_before  , (/nx,ny/) )
  call pool_push( rtm_integral_after   , (/nx,ny/) )
  call pool_push( thlm_integral_before , (/nx,ny/) )
  call pool_push( thlm_integral_after  , (/nx,ny/) )
  call pool_push( thlm_before          , (/nzm  /) )
  call pool_push( thlm_after           , (/nzm  /) )
  call pool_push( rtm_column           , (/nzm  /) )
#endif
!   allocate( t00      (ncrms)      )
!   allocate( tln      (plev)       )
!   allocate( qln      (plev)       )
