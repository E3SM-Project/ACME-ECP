...
subroutine crm(lchnk, icol, ncrms, dt_gl, plev, &
    ...
    real(crm_rknd), pointer, contiguous  :: colprec(:), colprecs(:)
    real(crm_rknd), pointer, contiguous  :: ustar(:), bflx(:), wnd(:)
    real(r8)      , pointer, contiguous  :: qtot (:,:)    ! Total water for water conservation check
    ...
    !!! Arrays
    real(crm_rknd), pointer, contiguous :: t00(:,:)
    real(crm_rknd), pointer, contiguous :: tln  (:,:)
    real(crm_rknd), pointer, contiguous :: qln  (:,:)
    real(crm_rknd), pointer, contiguous :: qccln(:,:)
    real(crm_rknd), pointer, contiguous :: qiiln(:,:)
    real(crm_rknd), pointer, contiguous :: uln  (:,:)
    real(crm_rknd), pointer, contiguous :: vln  (:,:)
#if defined(SP_ESMT)
    real(crm_rknd), pointer, contiguous  :: uln_esmt(:,:)
    real(crm_rknd), pointer, contiguous  :: vln_esmt(:,:)     ! tempoerary variables for expliciit scalar momentum transport
#endif
    real(crm_rknd), pointer, contiguous  :: cwp     (:,:,:)
    real(crm_rknd), pointer, contiguous  :: cwph    (:,:,:)
    real(crm_rknd), pointer, contiguous  :: cwpm    (:,:,:)
    real(crm_rknd), pointer, contiguous  :: cwpl    (:,:,:)
    logical       , pointer, contiguous  :: flag_top(:,:,:)
    real(crm_rknd), pointer, contiguous  :: cltemp  (:,:,:)
    real(crm_rknd), pointer, contiguous  :: cmtemp  (:,:,:)
    real(crm_rknd), pointer, contiguous  :: chtemp  (:,:,:)
    real(crm_rknd), pointer, contiguous  :: cttemp  (:,:,:)

    real(r8), pointer, contiguous :: dd_crm (:,:)     ! mass entraiment from downdraft
    real(r8), pointer, contiguous :: mui_crm(:,:)     ! mass flux up at the interface
    real(r8), pointer, contiguous :: mdi_crm(:,:)     ! mass flux down at the interface
    ...
    call gator_init(nx*ny*nz*ncrms*100)

  !-----------------------------------------------------------------------------------------------
  !-----------------------------------------------------------------------------------------------

  call gator_allocate( t00,[ncrms,nz] )
  call gator_allocate( tln,[ncrms,plev] )
  call gator_allocate( qln,[ncrms,plev] )
  call gator_allocate( qccln,[ncrms,plev] )
  call gator_allocate( qiiln,[ncrms,plev] )
  call gator_allocate( uln,[ncrms,plev] )
  call gator_allocate( vln,[ncrms,plev] )
#if defined(SP_ESMT)
  call gator_allocate( uln_esmt,[plev,ncrms] )
  call gator_allocate( vln_esmt,[plev,ncrms] )
#endif
  call gator_allocate( cwp,[ncrms,nx,ny] )
  call gator_allocate( cwph,[ncrms,nx,ny] )
  call gator_allocate( cwpm,[ncrms,nx,ny] )
  call gator_allocate( cwpl,[ncrms,nx,ny] )
  call gator_allocate( flag_top,[ncrms,nx,ny] )
  call gator_allocate( cltemp,[ncrms,nx,ny] )
  call gator_allocate( cmtemp,[ncrms,nx,ny] )
  call gator_allocate( chtemp,[ncrms,nx,ny] )
  call gator_allocate( cttemp,[ncrms,nx,ny] )
  call gator_allocate( dd_crm ,[ncrms,plev]   )
  call gator_allocate( mui_crm,[ncrms,plev+1] )
  call gator_allocate( mdi_crm,[ncrms,plev+1] )
  call gator_allocate( ustar,[ncrms] )
  call gator_allocate( bflx,[ncrms] )
  call gator_allocate( wnd,[ncrms] )
  call gator_allocate( qtot ,[ncrms,20] )
  call gator_allocate( colprec ,[ncrms] )
  call gator_allocate( colprecs,[ncrms] )
...
!--------------------------------------------------
  ...
  call gator_deallocate( t00)
  call gator_deallocate( tln)
  call gator_deallocate( qln)
  call gator_deallocate( qccln)
  call gator_deallocate( qiiln)
  call gator_deallocate( uln)
  call gator_deallocate( vln)
#if defined(SP_ESMT)
  call gator_deallocate( uln_esmt)
  call gator_deallocate( vln_esmt)
#endif
  call gator_deallocate( cwp)
  call gator_deallocate( cwph)
  call gator_deallocate( cwpm)
  call gator_deallocate( cwpl)
  call gator_deallocate( flag_top)
  call gator_deallocate( cltemp)
  call gator_deallocate( cmtemp)
  call gator_deallocate( chtemp)
  call gator_deallocate( cttemp)
#ifdef CLUBB_CRM
  call gator_deallocate( rtm_integral_before )
  call gator_deallocate( rtm_integral_after )
  call gator_deallocate( thlm_integral_before)
  call gator_deallocate( thlm_integral_after)
  call gator_deallocate( thlm_before)
  call gator_deallocate( thlm_after)
  call gator_deallocate( rtm_column)
#endif /* CLUBB_CRM */
  call gator_deallocate( dd_crm  )
  call gator_deallocate( mui_crm )
  call gator_deallocate( mdi_crm )
  call gator_deallocate( ustar )
  call gator_deallocate( bflx )
  call gator_deallocate( wnd )
  call gator_deallocate( qtot )
  call gator_deallocate( colprec  )
  call gator_deallocate( colprecs )
