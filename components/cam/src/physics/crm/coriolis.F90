      ...
      do k=1,nzm
        ...
        do j=1,ny
          ...
          do i=1,nx
            ...
            dudt(icrm,i,j,k,na)=dudt(icrm,i,j,k,na)+fcory(j)*(v_av-vg0(icrm,k))-fcorzy(j)*w_av
            u_av=0.25*(u(icrm,i,j,k)+u(icrm,ic,j,k)+u(icrm,i,jb,k)+u(icrm,ic,jb,k))
            dvdt(icrm,i,j,k,na)=dvdt(icrm,i,j,k,na)-0.5*(fcory(j)+fcory(jb))*(u_av-ug0(icrm,k))
          end do ! i
        end do ! j
      end do ! k
      ...
      do k=1,nzm
        ...
        do j=1,ny
          do i=1,nx
            ...
            ic=i+1
            w_av=0.25*(w(icrm,i,j,kc)+w(icrm,ib,j,kc)+w(icrm,i,j,k)+w(icrm,ib,j,k))
            dudt(icrm,i,j,k,na)=dudt(icrm,i,j,k,na)+fcory(j)*(v(icrm,i,j,k)-vg0(icrm,k))-fcorzy(j)*w_av
            dvdt(icrm,i,j,k,na)=dvdt(icrm,i,j,k,na)-fcory(j)*(u(icrm,i,j,k)-ug0(icrm,k))
          end do ! i
        end do ! i
      end do ! k
