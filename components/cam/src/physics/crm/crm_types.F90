...
contains
   ...
   !------------------------------------------------------------------------------------------------
#if defined( ECPP )
   subroutine crm_ecpp_initialize(this, ncrms, plev)
      use ecppvars, only: NCLASS_CL, ncls_ecpp_in, NCLASS_PR
      class(crm_ecpp_type), intent(inout) :: this
      integer, intent(in) :: ncrms, plev
      if (.not.allocated(this% )) allocate(acen            (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(acen_tf         (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(rhcen           (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(qcloudcen       (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(qicecen         (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(qlsinkcen       (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(precrcen        (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(precsolidcen    (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(qlsink_afcen    (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(qlsink_bfcen    (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(qlsink_avgcen   (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(praincen        (ncrms,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)  
      if (.not.allocated(this% )) allocate(wwqui_cen       (ncrms,plev)
      if (.not.allocated(this% )) allocate(wwqui_cloudy_cen(ncrms,plev)
      if (.not.allocated(this% )) allocate(abnd            (ncrms,plev+1,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)
      if (.not.allocated(this% )) allocate(abnd_tf         (ncrms,plev+1,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)
      if (.not.allocated(this% )) allocate(massflxbnd      (ncrms,plev+1,NCLASS_CL,ncls_ecpp_in,NCLASS_PR)
      if (.not.allocated(this% )) allocate(wupthresh_bnd   (ncrms,plev+1)
      if (.not.allocated(this% )) allocate(wdownthresh_bnd (ncrms,plev+1)
      if (.not.allocated(this% )) allocate(wwqui_bnd       (ncrms,plev+1)
      if (.not.allocated(this% )) allocate(wwqui_cloudy_bnd(ncrms,plev+1)
