      ...
      do j=1,ny
        do i=1,nx
          do icrm = 1 , ncrms
            ...
            fluxbu(icrm,i,j) = -(0.5*(u(icrm,i+1,j,1)+u(icrm,i,j,1))+ug)/u_h0*tau00 !+  uhl(icrm)/wspd*tau00   
            !print *, -(0.5*(u(icrm,i+1,j,1)+u(icrm,i,j,1))+ug)/u_h0*tau00,u_h0, ug, uhl(icrm)/wspd*tau00
            fluxbv(icrm,i,j) = -(0.5*(v(icrm,i,j+YES3D,1)+v(icrm,i,j,1))+vg)/u_h0*tau00 !+ vhl(icrm)/wspd*tau00 
            tmp = fluxbu(icrm,i,j)/dble(nx*ny)
