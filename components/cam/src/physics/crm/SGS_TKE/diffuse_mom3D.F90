...
contains
  ...
  subroutine diffuse_mom3D(ncrms,grdf_x, grdf_y, grdf_z, dimx1_d, dimx2_d, dimy1_d, dimy2_d, tk)
    ...
    real(crm_rknd) :: fu(0:nx,0:ny,nz)
    real(crm_rknd) :: fv(0:nx,0:ny,nz)
    real(crm_rknd) :: fw(0:nx,0:ny,nz)
    ...
    do icrm = 1 , ncrms
      ...
      do k=1,nzm
        ...
        do j=1,ny
          jb=j-1
          do i=1,nx
            dudt(i,j,k,na(icrm),icrm)=dudt(i,j,k,na(icrm),icrm)-(fu(i,j,k)-fu(i,jb,k))
            dvdt(i,j,k,na(icrm),icrm)=dvdt(i,j,k,na(icrm),icrm)-(fv(i,j,k)-fv(i,jb,k))
            dwdt(i,j,kc,na(icrm),icrm)=dwdt(i,j,kc,na(icrm),icrm)-(fw(i,j,k)-fw(i,jb,k))
          end do
        end do
