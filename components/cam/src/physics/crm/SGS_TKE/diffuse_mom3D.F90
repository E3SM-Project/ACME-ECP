  ...
  subroutine diffuse_mom3D(grdf_x, grdf_y, grdf_z, dimx1_d, dimx2_d, dimy1_d, dimy2_d, tk, ncrms, icrm)
    ...
    do k=1,nzm
      ...
      do j=1,ny
        jb=j-1
        do i=1,nx
          dudt(icrm,i,j,k,na)=dudt(icrm,i,j,k,na)-(fu(i,j,k)-fu(i,jb,k))
          dvdt(icrm,i,j,k,na)=dvdt(icrm,i,j,k,na)-(fv(i,j,k)-fv(i,jb,k))
          dwdt(icrm,i,j,kc,na)=dwdt(icrm,i,j,kc,na)-(fw(i,j,k)-fw(i,jb,k))
        end do
      end do
    ...
    do j=1,ny
      do i=1,nx
        tkz=rdz2*grdf_z(nzm)*tk(icrm,i,j,nzm)
