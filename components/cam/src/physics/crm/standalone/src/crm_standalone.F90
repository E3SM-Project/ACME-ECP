...
program crm_standalone
...
#endif
  allocate( ps(ncols) ) 
  allocate( pmid(ncols,plev) ) 
  allocate( pdel(ncols,plev) ) 
  allocate( phis(ncols) ) 
  allocate( zmid(ncols,plev) ) 
  allocate( zint(ncols,plev+1) )
  allocate( qrad_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
  ...
  allocate( tl(ncols,plev) ) 
  allocate( ql(ncols,plev) ) 
  allocate( qccl(ncols,plev) )
  allocate( qiil(ncols,plev) )
  allocate( ul(ncols,plev) ) 
  allocate( vl(ncols,plev) ) 
#ifdef CLUBB_CRM
  allocate( clubb_buffer(ncols,crm_nx, crm_ny, crm_nz+1,1:nclubbvars) )
  allocate( crm_cld(ncols,crm_nx, crm_ny, crm_nz+1) )
  allocate( clubb_tk(ncols,crm_nx, crm_ny, crm_nz) )
  allocate( clubb_tkh(ncols,crm_nx, crm_ny, crm_nz) )
  allocate( relvar(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( accre_enhan(ncols,crm_nx, crm_ny, crm_nz) )
  allocate( qclvar(ncols,crm_nx, crm_ny, crm_nz) )
#endif
  allocate( crm_tk(ncols,crm_nx, crm_ny, crm_nz) )
  allocate( crm_tkh(ncols,crm_nx, crm_ny, crm_nz) )
  ...
  allocate( w_crm(ncols,crm_nx,crm_ny,crm_nz) ) 
  allocate( t_crm(ncols,crm_nx,crm_ny,crm_nz) ) 
  allocate( micro_fields_crm(ncols,crm_nx,crm_ny,crm_nz,nmicro_fields+1) )
  allocate( qltend(ncols,plev) ) 
  allocate( qcltend(ncols,plev) )
  allocate( qiltend(ncols,plev) )
  allocate( t_rad(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( qv_rad(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( qc_rad(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( qi_rad(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( cld_rad(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( cld3d_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
#ifdef m2005
  allocate( nc_rad(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( ni_rad(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( qs_rad(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( ns_rad(ncols,crm_nx, crm_ny, crm_nz) )
  allocate( wvar_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( aut_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( acc_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( evpc_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( evpr_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( mlt_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( sub_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( dep_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( con_crm(ncols,crm_nx, crm_ny, crm_nz) ) 
  allocate( aut_crm_a(ncols,plev) ) 
  allocate( acc_crm_a(ncols,plev) ) 
  allocate( evpc_crm_a(ncols,plev) ) 
  allocate( evpr_crm_a(ncols,plev) ) 
  allocate( mlt_crm_a(ncols,plev) ) 
  allocate( sub_crm_a(ncols,plev) ) 
  allocate( dep_crm_a(ncols,plev) ) 
  allocate( con_crm_a(ncols,plev) ) 
#endif
  allocate( precc(ncols) ) 
  allocate( precl(ncols) ) 
  allocate( cld(ncols,plev) )  
  allocate( cldtop(ncols,plev) )  
  allocate( gicewp(ncols,plev) )  
  allocate( gliqwp(ncols,plev) )  
  allocate( mc(ncols,plev) )   
  allocate( mcup(ncols,plev) ) 
  allocate( mcdn(ncols,plev) ) 
  allocate( mcuup(ncols,plev) ) 
  allocate( mcudn(ncols,plev) ) 
  allocate( crm_qc(ncols,plev) )  
  allocate( crm_qi(ncols,plev) )  
  allocate( crm_qs(ncols,plev) )  
  allocate( crm_qg(ncols,plev) )  
  allocate( crm_qr(ncols,plev) )  
#ifdef m2005
  allocate( crm_nc(ncols,plev) )  
  allocate( crm_ni(ncols,plev) )  
  allocate( crm_ns(ncols,plev) )  
  allocate( crm_ng(ncols,plev) )  
  allocate( crm_nr(ncols,plev) )  
#ifdef MODAL_AERO
  allocate( naermod(ncols,plev, ntot_amode) )     
  allocate( vaerosol(ncols,plev, ntot_amode) )    
  allocate( hygro(ncols,plev, ntot_amode) )       
#endif 
#endif
  allocate( mu_crm(ncols,plev) )             
  allocate( md_crm(ncols,plev) )             
  allocate( du_crm(ncols,plev) )             
  allocate( eu_crm(ncols,plev) )             
  allocate( ed_crm(ncols,plev) )             
  allocate( dd_crm(ncols,plev) )             
  allocate( jt_crm(ncols) )                   
  allocate( mx_crm(ncols) )                   
  allocate( mui_crm(ncols,plev+1) )             
  allocate( mdi_crm(ncols,plev+1) )             
  allocate( flux_qt(ncols,plev) ) 
  allocate( fluxsgs_qt(ncols,plev) ) 
  allocate( tkez(ncols,plev) ) 
  allocate( tkesgsz(ncols,plev) ) 
  allocate( tkz(ncols,plev) )  
  allocate( flux_u(ncols,plev) ) 
  allocate( flux_v(ncols,plev) ) 
  allocate( flux_qp(ncols,plev) ) 
  allocate( pflx(ncols,plev) )    
  allocate( qt_ls(ncols,plev) ) 
  allocate( qt_trans(ncols,plev) )
  allocate( qp_trans(ncols,plev) ) 
  allocate( qp_fall(ncols,plev) ) 
  allocate( qp_src(ncols,plev) ) 
  allocate( qp_evp(ncols,plev) ) 
  allocate( t_ls(ncols,plev) ) 
  ...
  allocate( qc_crm(ncols,crm_nx, crm_ny, crm_nz) )
  allocate( qi_crm(ncols,crm_nx, crm_ny, crm_nz) )
  allocate( qpc_crm(ncols,crm_nx, crm_ny, crm_nz) )
  allocate( qpi_crm(ncols,crm_nx, crm_ny, crm_nz) )
  allocate( prec_crm(ncols,crm_nx, crm_ny) )
#ifdef ECPP
  allocate( acen(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) )   
  allocate( acen_tf(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) ) 
  allocate( rhcen(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) )  
  allocate( qcloudcen(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) )  
  allocate( qicecen(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) ) 
  allocate( qlsinkcen(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) )  
  allocate( precrcen(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) )   
  allocate( precsolidcen(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) )   
  allocate( qlsink_bfcen(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) )  
  allocate( qlsink_avgcen(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) )  
  allocate( praincen(ncols,plev,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) )  
  allocate( wwqui_cen(ncols,plev) )                                
  allocate( wwqui_cloudy_cen(ncols,plev) )                         
  allocate( abnd(ncols,plev+1,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) )   
  allocate( abnd_tf(ncols,plev+1,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) ) 
  allocate( massflxbnd(ncols,plev+1,NCLASS_CL,ncls_ecpp_in,NCLASS_PR) ) 
  allocate( wupthresh_bnd(ncols,plev+1) )             
  allocate( wdownthresh_bnd(ncols,plev+1) )           
  allocate( wwqui_bnd(ncols,plev+1) )                                
  allocate( wwqui_cloudy_bnd(ncols,plev+1) )                         
#endif
  allocate( qtot(ncols,20) )
  ...
  do i = 1 , ncols
    call dmdf_read(lchnk           (i:i                 ),trim(fname_in),'lchnk'           ,col1-1+i,col1-1+i,.true. ,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(icol            (i:i                 ),trim(fname_in),'icol'            ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(latitude0       (i:i                 ),trim(fname_in),'latitude0'       ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(longitude0      (i:i                 ),trim(fname_in),'longitude0'      ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(ps              (i:i                 ),trim(fname_in),'ps'              ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(pmid            (i:i,:               ),trim(fname_in),'pmid'            ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(pdel            (i:i,:               ),trim(fname_in),'pdel'            ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(phis            (i:i                 ),trim(fname_in),'phis'            ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(zmid            (i:i,:               ),trim(fname_in),'zmid'            ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(zint            (i:i,:               ),trim(fname_in),'zint'            ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(qrad_crm        (i:i,:,:,:           ),trim(fname_in),'qrad_crm'        ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(dt_gl           (i:i                 ),trim(fname_in),'dt_gl'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(ocnfrac         (i:i                 ),trim(fname_in),'ocnfrac'         ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(tau00           (i:i                 ),trim(fname_in),'tau00'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(wndls           (i:i                 ),trim(fname_in),'wndls'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(bflxls          (i:i                 ),trim(fname_in),'bflxls'          ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(fluxu00         (i:i                 ),trim(fname_in),'fluxu00'         ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(fluxv00         (i:i                 ),trim(fname_in),'fluxv00'         ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(fluxt00         (i:i                 ),trim(fname_in),'fluxt00'         ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(fluxq00         (i:i                 ),trim(fname_in),'fluxq00'         ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(tl              (i:i,:               ),trim(fname_in),'tl'              ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(ql              (i:i,:               ),trim(fname_in),'ql'              ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(qccl            (i:i,:               ),trim(fname_in),'qccl'            ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(qiil            (i:i,:               ),trim(fname_in),'qiil'            ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(ul              (i:i,:               ),trim(fname_in),'ul'              ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(vl              (i:i,:               ),trim(fname_in),'vl'              ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
#ifdef CLUBB_CRM
    call dmdf_read(clubb_buffer    (i:i,:,:,:,:         ),trim(fname_in),'clubb_buffer'    ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
#endif
    call dmdf_read(cltot           (i:i                 ),trim(fname_in),'cltot'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(clhgh           (i:i                 ),trim(fname_in),'clhgh'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(clmed           (i:i                 ),trim(fname_in),'clmed'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(cllow           (i:i                 ),trim(fname_in),'cllow'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(u_crm           (i:i,:,:,:           ),trim(fname_in),'u_crm'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(v_crm           (i:i,:,:,:           ),trim(fname_in),'v_crm'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(w_crm           (i:i,:,:,:           ),trim(fname_in),'w_crm'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(t_crm           (i:i,:,:,:           ),trim(fname_in),'t_crm'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(micro_fields_crm(i:i,:,:,:,:         ),trim(fname_in),'micro_fields_crm',col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
#ifdef m2005
#ifdef MODAL_AERO
    call dmdf_read(naermod         (i:i,:,:             ),trim(fname_in),'naermod'         ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(vaerosol        (i:i,:,:             ),trim(fname_in),'vaerosol'        ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(hygro           (i:i,:,:             ),trim(fname_in),'hygro'           ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
#endif
#endif
    call dmdf_read(dd_crm          (i:i,:               ),trim(fname_in),'dd_crm'          ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(mui_crm         (i:i,:               ),trim(fname_in),'mui_crm'         ,col1-1+i,col1-1+i,.false.,.false.); _ERR(success,error_string,__LINE__)
    call dmdf_read(mdi_crm         (i:i,:               ),trim(fname_in),'mdi_crm'         ,col1-1+i,col1-1+i,.false.,.true. ); _ERR(success,error_string,__LINE__)
  enddo
