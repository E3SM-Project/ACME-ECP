...
contains
  ...
  subroutine advect_scalar (f,fadv,flux,f2leadv,f2legrad,fwleadv,doit,ncrms,icrm)
    ...
    if(RUN3D) then
      call advect_scalar3D(f, u, v, w, rho, rhow, flux, ncrms, icrm)
    else
      call advect_scalar2D(f, u, w, rho, rhow, flux, ncrms, icrm)
    endif
