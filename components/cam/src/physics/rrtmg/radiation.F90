...
!===============================================================================
  ...
  subroutine radiation_tend( state, ptend,pbuf, &
    ...
    logical :: last_column
    integer :: ii,jj,m,i_rad,j_rad,icol
#ifdef MAML1
    ...
    real(r8) :: solsd_col(pcols, crm_nx_rad, crm_ny_rad) 
    real(r8) :: solld_col(pcols, crm_nx_rad, crm_ny_rad) 
    real(r8) :: fsns_col(pcols, crm_nx_rad, crm_ny_rad) 
    real(r8) :: flwds_col(pcols, crm_nx_rad, crm_ny_rad) 
    real(r8) :: sols_loc(pcols)
    real(r8) :: sols_loc(pcols)
    real(r8) :: soll_loc(pcols)
...
!----------------------------------------------------------------------
    ...
    if (dosw .or. dolw) then
      ...
      ! over column groups
      ![lee1046] Packing and Unpacking for the CRM radiation column level can be
      !avoided when num_inst_atm = crm_nx_rad (TBD)
      if(use_SPCAM .and. use_MAML) then 
         ...
         do jj=1,crm_ny
           do ii=1,num_inst_atm
             do icol = 1 , ncol
               ...
               aldir_col(icol,i_rad,j_rad) = aldir_col(icol,i_rad,j_rad) + cam_in%aldir(icol,ii)
               aldif_col(icol,i_rad,j_rad) = aldif_col(icol,i_rad,j_rad) + cam_in%aldif(icol,ii)
               asdir_col(icol,i_rad,j_rad) = asdir_col(icol,i_rad,j_rad) + cam_in%asdir(icol,ii)
               asdif_col(icol,i_rad,j_rad) = asdif_col(icol,i_rad,j_rad) + cam_in%asdif(icol,ii)
             end do ! icol = 1, ncol
         ...
         aldir_col(1:ncol,:,:) = cam_in%aldir(1:ncol,1)
         aldif_col(1:ncol,:,:) = cam_in%aldif(1:ncol,1)
         asdir_col(1:ncol,:,:) = cam_in%asdir(1:ncol,1)
         asdif_col(1:ncol,:,:) = cam_in%asdif(1:ncol,1)
      end if
      ...
      do jj=1,crm_ny_rad
        do ii=1,crm_nx_rad
          ...
          ! [lee1046] I don't see 'tint' being used anywhere inside radiation.F90.
            ...
            do icall = N_DIAG, 0, -1
              ...
              if (active_calls(icall)) then
                ...
                call rad_rrtmg_sw( &
...
#ifdef MAML1
                     cam_in%asdir(:ncol,ii),     cam_in%asdif(:ncol,ii),&
                     cam_in%aldir(:ncol,ii),     cam_in%aldif(:ncol,ii), &
#else
                     ...
                     fsnsc,        fsdsc,        fsds,                                       &
                     sols_loc,     soll_loc,     solsd_loc,    solld_loc,                    &
                     fns,          fcns,                                                     &
...
#else
                         ![lee1046]: when *_col are passed down to cam_out, the
...
![lee1046] lwupcgs is not being used inside radiation.F90. Commenting out this
!block for now
...
!            end do
      ...
      ![lee1046] Unpack sols_co, soll_col, solsd_col, solld_col, flwds_col to be
      ...
      do jj = 1, crm_ny
         do ii = 1, crm_nx
            do icol = 1, ncol
               ...
               j_rad = (jj-1) / (crm_ny/crm_ny_rad) + 1
               cam_out%sols(icol,ii) = sols_col(icol,i_rad_,j_rad)
               cam_out%soll(icol,ii) = soll_col(icol,i_rad_,j_rad)
               cam_out%solsd(icol,ii) = solsd_col(icol,i_rad,j_rad)
               cam_out%solld(icol,ii) = solld_col(icol,i_rad,j_rad)
               cam_out%solld(icol,ii) = solld_col(icol,i_rad,j_rad)
               cam_out%flwds(icol,ii) = flwds_col(icol,i_rad,j_rad)
            end do! icol = 1,ncol
         end do ! ii = 1,crm_nx
      end do ! jj = 1, crm_ny
    ...
    ![lee1046] asdir_glb is not used inside radheat_tend. 
    !Therefore below modification is unnecessary. 
    asdir_glb = 0.
    asdir_glb(1:ncol) = sum(cam_in%asdir(1:ncol,1:num_inst_atm),dim=2)/real(num_inst_atm,r8)
    call radheat_tend(state, pbuf,  ptend, qrl, qrs, fsns, &
                      fsnt, flns, flnt, asdir_glb, net_flx)
