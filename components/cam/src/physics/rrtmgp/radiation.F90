...
contains
   ...
   !-------------------------------------------------------------------------------
      ...
      call get_sw_spectral_midpoints(sw_band_midpoints, 'meters')
      call get_lw_spectral_midpoints(lw_band_midpoints, 'meters')
      ...
      if (use_SPCAM) then
         ...
         call addfld ('CRM_CLD_RAD', (/'crm_nx_rad','crm_ny_rad','crm_nz'/), 'I', 'fraction', &
                      'CRM cloud fraction')
         call addfld ('CRM_QV_RAD', (/'crm_nx_rad','crm_ny_rad','crm_nz'/), 'I', 'kg/kg', &
                      'CRM water vapor')
         call addfld ('CRM_QC_RAD', (/'crm_nx_rad','crm_ny_rad','crm_nz'/), 'I', 'kg/kg', &
                      'CRM cloud water')
         call addfld ('CRM_QI_RAD', (/'crm_nx_rad','crm_ny_rad','crm_nz'/), 'I', 'kg/kg', &
                      'CRM cloud ice')
         call addfld ('CRM_CLD_TAU_LW', (/'crm_nx_rad','crm_ny_rad','crm_nz','lwband'/), 'I', 'kg/kg', &
                      'CRM longwave cloud optical depth')
         call addfld ('CRM_LIQ_TAU_LW', (/'crm_nx_rad','crm_ny_rad','crm_nz','lwband'/), 'I', 'kg/kg', &
                      'CRM longwave liquid cloud optical depth')
         call addfld ('CRM_ICE_TAU_LW', (/'crm_nx_rad','crm_ny_rad','crm_nz','lwband'/), 'I', 'kg/kg', &
                      'CRM longwave ice cloud optical depth')
         call addfld ('CRM_AER_TAU_LW', (/'crm_nx_rad','crm_ny_rad','crm_nz','lwband'/), 'I', 'kg/kg', &
                      'CRM longwave aerosol optical depth')
         call addfld ('CRM_CLD_TAU_SW', (/'crm_nx_rad','crm_ny_rad','crm_nz','swband'/), 'I', 'kg/kg', &
                      'CRM shortwave cloud optical depth')
         call addfld ('CRM_AER_TAU_SW', (/'crm_nx_rad','crm_ny_rad','crm_nz','swband'/), 'I', 'kg/kg', &
                      'CRM shortwave aerosol optical depth')
         call addfld ('CRM_LIQ_TAU_SW', (/'crm_nx_rad','crm_ny_rad','crm_nz','swband'/), 'I', 'kg/kg', &
                      'CRM shortwave liquid cloud optical depth')
         call addfld ('CRM_ICE_TAU_SW', (/'crm_nx_rad','crm_ny_rad','crm_nz','swband'/), 'I', 'kg/kg', &
                      'CRM shortwave ice cloud optical depth')
      end if
   ...
   subroutine radiation_tend(state_in,ptend,    pbuf,          cam_out, cam_in,  &
      ...
      real(r8) :: &
         qrl_col(pcols,pver), qrlc_col(pcols,pver), &
         qrs_col(pcols,pver), qrsc_col(pcols,pver)

      ! Optical properties
      type(ty_optical_props_1scl) :: cloud_optics_lw, aerosol_optics_lw
      type(ty_optical_props_2str) :: cloud_optics_sw, aerosol_optics_sw

      ! CRM area factor for domain averages
      ...
      real(r8), pointer, dimension(:,:,:,:) :: &
         crm_t, crm_qv, crm_qc, crm_qi, crm_cld, crm_qrad
...
#endif
      real(r8), pointer :: dei(:,:), rel(:,:), rei(:,:)
      real(r8), dimension(pcols,pver) :: dei_save, rel_save, rei_save

      real(r8) :: cld_tau_sw(pcols,pver,nswbands)
      real(r8) :: cld_ssa_sw(pcols,pver,nswbands)
      real(r8) :: cld_asm_sw(pcols,pver,nswbands)
      real(r8) :: liq_tau_sw(pcols,pver,nswbands)
      real(r8) :: ice_tau_sw(pcols,pver,nswbands)
      real(r8) :: aer_tau_sw(pcols,pver,nswbands)
      real(r8) :: aer_ssa_sw(pcols,pver,nswbands)
      real(r8) :: aer_asm_sw(pcols,pver,nswbands)

      real(r8) :: cld_tau_lw(pcols,pver,nlwbands)
      real(r8) :: liq_tau_lw(pcols,pver,nlwbands)
      real(r8) :: ice_tau_lw(pcols,pver,nlwbands)
      real(r8) :: aer_tau_lw(pcols,pver,nlwbands)

      ! CRM optical properties for output
      real(r8) :: crm_cld_tau_lw(pcols,crm_nx_rad,crm_ny_rad,crm_nz,nlwbands)
      real(r8) :: crm_liq_tau_lw(pcols,crm_nx_rad,crm_ny_rad,crm_nz,nlwbands)
      real(r8) :: crm_ice_tau_lw(pcols,crm_nx_rad,crm_ny_rad,crm_nz,nlwbands)
      real(r8) :: crm_aer_tau_lw(pcols,crm_nx_rad,crm_ny_rad,crm_nz,nlwbands)
      
      real(r8) :: crm_cld_tau_sw(pcols,crm_nx_rad,crm_ny_rad,crm_nz,nswbands)
      real(r8) :: crm_liq_tau_sw(pcols,crm_nx_rad,crm_ny_rad,crm_nz,nswbands)
      real(r8) :: crm_ice_tau_sw(pcols,crm_nx_rad,crm_ny_rad,crm_nz,nswbands)
      real(r8) :: crm_aer_tau_sw(pcols,crm_nx_rad,crm_ny_rad,crm_nz,nswbands)
      ...
      real(r8) :: albedo_direct(nswbands,pcols)
      real(r8) :: albedo_diffuse(nswbands,pcols)
      ...
      if (radiation_do('sw') .or. radiation_do('lw')) then
         ...
         if (use_SPCAM) then
...
#endif
            ! Output CRM cloud radiative properties
            call outfld('CRM_QV_RAD', crm_qv, state%ncol, state%lchnk)
            call outfld('CRM_QC_RAD', crm_qc, state%ncol, state%lchnk)
            call outfld('CRM_QI_RAD', crm_qi, state%ncol, state%lchnk)
            ...
            ! needed because the CAM optics routines bury pbuf down deep in the
            ! call stack, so we need to overwrite with CRM state here in order to
            ! use CRM information to calculate optics.
            iclwp_save = iclwp
            iciwp_save = iciwp
            cld_save   = cld
            dei_save   = dei
         ...
         ! Initialize optics and fluxes
         call t_startf('rad_allocate_memory')
         if (radiation_do('sw')) then
            call handle_error(cloud_optics_sw%alloc_2str(ncol, nlev_rad, k_dist_sw, name='sw cloud optics'))
            call handle_error(aerosol_optics_sw%alloc_2str(ncol, nlev_rad, k_dist_sw%get_band_lims_wavenumber(), name='sw aerosol optics'))
         ...
         if (radiation_do('lw')) then
            ...
            call initialize_rrtmgp_fluxes(ncol, nlev_rad+1, nlwbands, fluxes_lw_allsky, do_direct=.false.)
            call initialize_rrtmgp_fluxes(ncol, nlev_rad+1, nlwbands, fluxes_lw_clrsky, do_direct=.false.)
            call initialize_rrtmgp_fluxes(ncol, nlev_rad+1, nlwbands, fluxes_lw_allsky_col, do_direct=.false.)
            call initialize_rrtmgp_fluxes(ncol, nlev_rad+1, nlwbands, fluxes_lw_clrsky_col, do_direct=.false.)
         end if
         ...
         call t_startf('rad_diag_loop')
         call rad_cnst_get_call_list(active_calls)
         do icall = N_DIAG,0,-1
            if (active_calls(icall)) then

               ! Reset means for each diag call
               if (radiation_do('sw')) then
               ...
               do iy = 1,crm_ny_rad
                  do ix = 1,crm_nx_rad
                     ...
                     ! Overwrite state and pbuf with CRM for this column so that
                     ! we can calculate optics using CRM rather than GCM data
                     if (use_SPCAM) then
                     ...
                     if (radiation_do('sw')) then
                        ...
                        call set_cloud_optics_sw(state, pbuf, k_dist_sw, cld_tau_sw, cld_ssa_sw, cld_asm_sw, cloud_optics_sw)
                        ...
                        call calculate_fluxes_sw(pmid(1:ncol,1:nlev_rad), pint(1:ncol,1:nlev_rad+1), &
                                                 tmid(1:ncol,1:nlev_rad), tint(1:ncol,1:nlev_rad+1), &
                                                 coszrs, day_indices, &
                     ...
                     if (radiation_do('lw')) then
                        ...
                        call set_cloud_optics_lw(state, pbuf, k_dist_lw, cld_tau_lw, cloud_optics_lw)
                        ...
                        call calculate_fluxes_lw(pmid(1:ncol,1:nlev_rad), pint(1:ncol,1:nlev_rad+1), &
                                                 tmid(1:ncol,1:nlev_rad), tint(1:ncol,1:nlev_rad+1), &
                                                 gas_concentrations, &
         ...
         call t_stopf('rad_diag_loop')

         ! Restore pbuf fields to original values if we modified to hold CRM
         ! data for MMF
         if (use_SPCAM) then
            iclwp = iclwp_save
            iciwp = iciwp_save
      ...
      ! If we updated shortwave fluxes, then we need to update output surface
      ! and net fluxes, send fields to the history buffer, and free memory
      if (radiation_do('sw')) then
         ...
         ! If we did not do radiation this timestep, we need to convert heating
         ! rates from previous timestep here
      ...
      ! If we updated longwave fluxes, then we need to update output surface
      ! and net fluxes, send fields to the history buffer, and free memory
      if (radiation_do('lw')) then
   ...
   subroutine calculate_fluxes_sw(pmid, pint, tmid, tint, &
                                  coszrs, day_indices, &
                                  albedo_direct, albedo_diffuse, &
                                  gas_concentrations, &
                                  cloud_optics_sw, aerosol_optics_sw, &
      ...
      real(r8), intent(in) :: albedo_direct(:,:), albedo_diffuse(:,:)
      
      ! Temporaries for compressing to daytime-only
      type(ty_fluxes_byband) :: fluxes_allsky_day, fluxes_clrsky_day
      type(ty_gas_concs) :: gas_concentrations_day
      type(ty_optical_props_2str) :: aerosol_optics_sw_day, cloud_optics_sw_day
      real(r8), dimension(pcols,nlev_rad) :: tmid_day, pmid_day
      real(r8), dimension(pcols,nlev_rad+1) :: pint_day, tint_day
      real(r8) :: albedo_direct_day(nswbands,pcols), albedo_diffuse_day(nswbands,pcols)
      real(r8) :: coszrs_day(pcols)
      ...
      ncol = size(cloud_optics_sw%tau, 1)
      ...
      call t_startf('rad_compress_day_inputs_sw')
      call handle_error(cloud_optics_sw_day%alloc_2str( &
         nday, nlev_rad, k_dist_sw, name='sw cloud optics' &
      ))
      call handle_error(aerosol_optics_sw_day%alloc_2str( &
         nday, nlev_rad, k_dist_sw%get_band_lims_wavenumber(), name='sw aerosol optics' &
      ))
      do iday = 1,nday
         icol = day_indices(iday)
         cloud_optics_sw_day%tau(iday,:,:) = cloud_optics_sw%tau(icol,:,:)
         cloud_optics_sw_day%ssa(iday,:,:) = cloud_optics_sw%ssa(icol,:,:)
         cloud_optics_sw_day%g  (iday,:,:) = cloud_optics_sw%g  (icol,:,:)
      ...
      call t_stopf('rad_compress_day_inputs_sw')
      ...
      call t_stopf('rad_expand_day_fluxes_sw')

      ! Free temporary fluxes and optical properties for daytime-only
      call free_optics_sw(cloud_optics_sw_day)
      call free_optics_sw(aerosol_optics_sw_day)
   ...
   subroutine calculate_fluxes_lw(pmid, pint, tmid, tint, &
                                  gas_concentrations, &
                                  cloud_optics_lw, aerosol_optics_lw, &
                                  fluxes_allsky, fluxes_clrsky)
      ...
      ! Number of physics columns in this "chunk"
      ncol = size(cloud_optics_lw%tau, 1)
