...
program test
  ...
  use mo_test_files_io, only: write_optical_prop_values
  implicit none
  ...
  integer, dimension(2,nbnd), parameter :: band2bnd = reshape((/ 1, 1, 2,  2,  3,  3 /), (/2,nbnd/))

  character(len = 128) :: error_msg

  type(ty_optical_props) :: template_gpt, template_bnd

  call stop_on_err(template_gpt%init(real(band2gpt, wp), band2gpt))
  call stop_on_err(template_bnd%init(eal(band2gpt, wp)))
...
contains
  ...
  subroutine test_increment_1scalar_by_1scalar
    ...
    call stop_on_err(a%alloc_1scl(template_gpt, ncol, nlay))
    call stop_on_err(b%alloc_1scl(template_gpt, ncol, nlay))
    a%tau = 1._wp
    b%tau = 2._wp
    call stop_on_err(b%increment(a))
    call prep_file(fileName, ncol, nlay)
    call write_optical_prop_values(fileName,a)
  end subroutine
  ...
  subroutine test_increment_1scalar_by_2stream
    ...
    call stop_on_err(a%alloc_1scl(template_gpt, ncol, nlay))
    call stop_on_err(b%alloc_2str(template_gpt, ncol, nlay))
    a%tau = 1._wp
    b%tau = 2._wp
    b%ssa = 0.7_wp
    b%g = 0.8_wp
  ...
  subroutine test_increment_1scalar_by_nstream
    ...
    call stop_on_err(a%alloc_1scl(template_gpt, ncol, nlay))
    call stop_on_err(b%alloc_nstr(template_gpt, nmom, ncol, nlay))
    a%tau = 1._wp
    b%tau = 2._wp
    b%ssa = 0.7_wp
    b%p = 0.8_wp
  ...
  subroutine test_increment_2stream_by_1scalar
    ...
    call stop_on_err(a%alloc_2str(template_gpt, ncol, nlay))
    call stop_on_err(b%alloc_1scl(template_gpt, ncol, nlay))
    a%tau = 2._wp
    a%ssa = 0.7_wp
    a%g = 0.8_wp
  ...
  subroutine test_increment_nstream_by_1scalar
    ...
    call stop_on_err(a%alloc_nstr(template_gpt, nmom, ncol, nlay))
    call stop_on_err(b%alloc_1scl(template_gpt, ncol, nlay))
    a%tau = 2._wp
    a%ssa = 0.7_wp
    a%p = 0.8_wp
  ...
  subroutine test_expand_and_increment_1scalar_by_1scalar
    ...
    call stop_on_err(a%alloc_1scl(template_gpt, ncol, nlay))
    call stop_on_err(b%alloc_1scl(template_bnd, ncol, nlay))
    a%tau = 1._wp
    b%tau = 2._wp
    call stop_on_err(b%increment(a))
    call prep_file(fileName, ncol, nlay)
  ...
  subroutine test_expand_and_increment_1scalar_by_2stream
    ...
    call stop_on_err(a%alloc_1scl(template_gpt, ncol, nlay))
    call stop_on_err(b%alloc_2str(template_bnd, ncol, nlay))
    a%tau = 1._wp
    b%tau = 2._wp
    b%ssa = 0.7_wp
    b%g = 0.8_wp
  ...
  subroutine test_expand_and_increment_1scalar_by_nstream
    ...
    call stop_on_err(a%alloc_1scl(template_gpt, ncol, nlay))
    call stop_on_err(b%alloc_nstr(template_bnd, nmom, ncol, nlay))
    a%tau = 1._wp
    b%tau = 2._wp
    b%ssa = 0.7_wp
    b%p = 0.8_wp
