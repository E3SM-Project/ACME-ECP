...
program test_two_stream
  ...
  if(do_sw) then
    ...
    call apply_BC(ncol, nlay, ngpt, logical(top_at_1, wl), toa_src, mu0, flux_dn_dir)
  else
    ...
    type is (ty_optical_props_2str)
      if(do_lw) then
        do k = 1, ngpt
         ...
         !
         call lw_combine_sources(ncol, nlay, logical(top_at_1, wl), &
                                 sources%lev_source_inc(:,:,k), sources%lev_source_dec(:,:,k), &
                                 lev_source)
         !
         ! Source function for diffuse radiation
         !
          call lw_source_2str(ncol, nlay, logical(top_at_1, wl), &
        ...
        do k = 1, ngpt
          ...
          call sw_source_2str(ncol, nlay, logical(top_at_1, wl), Rdir, Tdir, Tnoscat, &
