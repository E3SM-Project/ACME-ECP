...
module mo_lw_solver
  ...
  use mo_rte_solver_kernels, only: apply_BC, lw_solver_noscat, lw_solver_noscat_GaussQuad, lw_solver_2stream
  implicit none
  private
...
contains
  ...
  function lw_solver(ncol, nlay, ngpt, top_is_1, &
    ...
    if(present(inc_flux)) then
      call apply_BC(ncol, nlay, ngpt, logical(top_is_1, wl), inc_flux, flux_dn)
    else
      !
    ...
    select type (atmos)
      class is (ty_optical_props_1scl)
        ...
        if(present(Ds)) then
          ...
          !
          call lw_solver_noscat_GaussQuad(ncol, nlay, ngpt, logical(top_is_1, wl), &
      ...
      class is (ty_optical_props_2str)
        ...
        !
        call lw_solver_2stream(ncol, nlay, ngpt, logical(top_is_1, wl), &
                               atmos%tau, atmos%ssa, atmos%g,           &
