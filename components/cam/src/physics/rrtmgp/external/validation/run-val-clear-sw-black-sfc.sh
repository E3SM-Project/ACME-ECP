...
#
python change-values.py ../test/data/garand-atmospheres-sw.nc sfc_alb_direct      0. 
python change-values.py ../test/data/garand-atmospheres-sw.nc sfc_alb_diffuse     0. 
...
python ../test/util/scripts/run_tests.py --root .. --no_diff --test run-shdompp-lw-clear.ini
# Shortwave clear-sky fluxes (about 8 minutes)
python ../test/util/scripts/run_tests.py --root .. --no_diff --test run-shdompp-sw-clear.ini
...
python ../test/util/scripts/run_tests.py --root .. --no_diff --test run-rrtmgp-clear.ini

#
# Comparisons -- SHDOM/LBL, RRTMGP/LBL, RRTMGP/SHDOM, for both LW and SW 
#
cd data/
ln -sf charts-sw-inputs-outputs-clear_alb0_sza30.nc charts-sw-inputs-outputs-clear.nc
cd .. 
