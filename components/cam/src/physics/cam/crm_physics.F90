...
! local variables
  ...
  if (use_SPCAM) then
   ...
   ! CRM state fields. These have scope "global" because they need to persist across timesteps
   ...
   ! CRM fields that should be used to affect the radiation. Note that these
   ! might have a reduced horizontal extent (crm_nx_rad <= crm_nx) if we want to
   ! do reduced radiation. Also, these are (currently) time-averages, but we
   ! might want to change that in the future, in which case radiation should
   ! probably just use the CRM microphysics variables directly?
...
!------------------------------------------------------------
   ...
   call pbuf_get_field (pbuf, crm_qv_rad_idx,  crm_state%qv_rad)
   call pbuf_get_field (pbuf, crm_qc_rad_idx,  crm_state%qc_rad)
   call pbuf_get_field (pbuf, crm_qi_rad_idx,  crm_state%qi_rad)
   call pbuf_get_field (pbuf, crm_cld_rad_idx, crm_state%cld_rad)
   ...
   ! physics buffer; note that these are only used for outfld calls, but we need
   ! a real variable for the crm_state fields to point to.
   ...
   if(is_first_step()) then
      do i=1,ncol
         do k=1,crm_nz
            ...
            ! TODO: put all of this in an initialization routine
            !call populate_crm_state(state, pbuf, crm_state)
...
! TODO: do we need this ifdef anymore?
      ...
      ! TODO: I think these initializations can go away, since before these were
      ! intent(out) and now are initialized in the initialization routine.
      do k=1,crm_nz
         m = pver-k+1
         do i=1,ncol
            ...
            crm_state%qi_rad (i,:,:,k)      = 0.
            crm_state%cld_rad(i,:,:,k)      = 0.
#ifdef m2005
            if (SPCAM_microp_scheme .eq. 'm2005') then
               crm_state%nc_rad(i,:,:,k) = 0.0
               crm_state%ni_rad(i,:,:,k) = 0.0       
               crm_state%qs_rad(i,:,:,k) = 0.0
               crm_state%ns_rad(i,:,:,k) = 0.0
               crm_state%wvar(i,:,:,k) = 0.0
               ! hm 7/26/11, add new output
               crm_state%aut (i,:,:,k) = 0.0
               crm_state%acc (i,:,:,k) = 0.0
               crm_state%evpc(i,:,:,k) = 0.0
               crm_state%evpr(i,:,:,k) = 0.0
               crm_state%mlt (i,:,:,k) = 0.0
               crm_state%sub (i,:,:,k) = 0.0
               crm_state%dep (i,:,:,k) = 0.0
               crm_state%con (i,:,:,k) = 0.0
            endif
#endif
...
!===================================================================================
   ...
   !----------------------------------------------------------------------
   ! set the crm input variables
   !----------------------------------------------------------------------

   crm_input%zmid(:ncol,:pver)   = state%zm(:ncol,:pver)      
   crm_input%zint(:ncol,:pver+1) = state%zi(:ncol,:pver+1)
   crm_input%tl(:ncol,:pver)     = state%t(:ncol,:pver)
   crm_input%ql(:ncol,:pver)     = state%q(:ncol,:pver,1)
   crm_input%qccl(:ncol,:pver)   = state%q(:ncol,:pver,ixcldliq)
   crm_input%qiil(:ncol,:pver)   = state%q(:ncol,:pver,ixcldice)
   crm_input%ps(:ncol)           = state%ps(:ncol)
   crm_input%pmid(:ncol,:pver)   = state%pmid(:ncol,:pver)
   crm_input%pint(:ncol,:pver+1) = state%pint(:ncol,:pver+1)
...
!--------------------------------------------------------------------------------------------------
      ...
      ! NOTE: these don't seem to be used anywhere, other than to transfer to
      ! prec_dp and snow_dp?
      ...
      call pbuf_set_field(pbuf, ifld, crm_output%mu_crm(:ncol,:), start=(/1,1/), kount=(/ncol, pver/) )
      ifld = pbuf_get_index( 'MD_CRM' )
      call pbuf_set_field(pbuf, ifld, crm_output%md_crm(:ncol,:), start=(/1,1/), kount=(/ncol, pver/) )
      ifld = pbuf_get_index( 'EU_CRM' )
      call pbuf_set_field(pbuf, ifld, crm_output%eu_crm(:ncol,:), start=(/1,1/), kount=(/ncol, pver/) )
      ifld = pbuf_get_index( 'DU_CRM' )
      call pbuf_set_field(pbuf, ifld, crm_output%du_crm(:ncol,:), start=(/1,1/), kount=(/ncol, pver/) )
      ifld = pbuf_get_index( 'ED_CRM' )
      call pbuf_set_field(pbuf, ifld, crm_output%eu_crm(:ncol,:), start=(/1,1/), kount=(/ncol, pver/) )
      ifld = pbuf_get_index( 'JT_CRM' )
      call pbuf_set_field(pbuf, ifld, crm_output%jt_crm(:ncol), start=(/1/), kount=(/ncol/) )
      ifld = pbuf_get_index( 'MX_CRM' )
      call pbuf_set_field(pbuf, ifld, crm_output%mx_crm(:ncol), start=(/1/), kount=(/ncol/) )
...
#endif
      call crm_physics_out(lchnk, ncol, state, crm_state, crm_output, ptend, phys_stage)
#ifdef ECPP
      ! TODO: fold this into crm_physics_out
      if (use_ECPP) then
         crm_ecpp%qlsinkcen = crm_ecpp%qlsink_avgcen   ! TODO: move this somewhere else
         call crm_ecpp_out(lchnk,crm_ecpp)
      endif
#endif
   ...
   do  m=1,crm_nz
      do jj=1,crm_ny_rad
         do ii=1,crm_nx_rad
            do  i=1,ncol
               if(crm_state%qc_rad(i,ii,jj,m)+crm_state%qi_rad(i,ii,jj,m).le.1.0e-10) then
                  crm_state%cld_rad(i,ii,jj,m) = 0.0_r8
               endif
            enddo
         enddo
      enddo
...
subroutine crm_physics_out(lchnk, ncol, state, crm_state, crm_output, ptend, phys_stage)
   !------------------------------------------------------------------------------------------ 
   ! 
   ...
   real(r8), dimension(pcols)       :: tgicewp  ! Vertically integrated IWP (kg/m2)
   real(r8), dimension(pcols)       :: tgliqwp  ! Vertically integrated liquid water path (kg/m2)
   real(r8), dimension(pcols)       :: tgwp     ! Vertically integrated (total) cloud water path (kg/m2)
   real(r8), dimension(pcols, pver) :: cicewp   ! in-cloud cloud ice water path (kg/m2)
   real(r8), dimension(pcols, pver) :: cliqwp   ! in-cloud cloud liquid water path (kg/m2)
   real(r8), dimension(pcols, pver) :: gwp
   real(r8), dimension(pcols, pver) :: cwp
   real(r8), dimension(pcols, pver) :: cldtop_gcm
   ...
   call cnst_get_ind('CLDLIQ', ixcldliq)
   call cnst_get_ind('CLDICE', ixcldice)
   tgicewp   (1:ncol)   = 0._r8
   tgliqwp   (1:ncol)   = 0._r8
   tgwp      (1:ncol)   = 0._r8
   cicewp    (1:ncol,:) = 0._r8
   cliqwp    (1:ncol,:) = 0._r8
   gwp       (1:ncol,:) = 0._r8
   cwp       (1:ncol,:) = 0._r8
   cldtop_gcm(1:ncol,:) = 0._r8
   ...
   !!! sam1mom only:
   if (SPCAM_microp_scheme .eq. 'sam1mom') then
      call outfld('CRM_QV  ', crm_state%qt (1:ncol,:,:,:)  &
                             -crm_state%qcl(1:ncol,:,:,:)  &
                             -crm_state%qci(1:ncol,:,:,:), ncol, lchnk )
   endif
   ...
   if (SPCAM_microp_scheme .eq. 'm2005') then 
      call outfld('CRM_QV  ',crm_state%qt (1:ncol,:,:,:) &
                            -crm_state%qcl(1:ncol,:,:,:), ncol, lchnk )
      ...
      call outfld('SPNI    ',crm_output%ni_mean(1:ncol,:), ncol, lchnk )
      call outfld('SPNS    ',crm_output%ns_mean(1:ncol,:), ncol, lchnk )
      call outfld('SPNG    ',crm_output%ng_mean(1:ncol,:), ncol, lchnk )
      call outfld('SPNR    ',crm_output%nr_mean(1:ncol,:), ncol, lchnk )
   endif
#endif /* m2005 */
   ...
   call outfld('SPDQ    ', ptend%q(1:ncol,:,1)       , ncol, lchnk )
   call outfld('SPDQC   ', ptend%q(1:ncol,:,ixcldliq), ncol, lchnk )
   call outfld('SPDQI   ', ptend%q(1:ncol,:,ixcldice), ncol, lchnk )
#ifdef SP_USE_ESMT
   call outfld('U_ESMT'  , ptend%u(1:ncol,:), ncol, lchnk )
   call outfld('V_ESMT'  , ptend%v(1:ncol,:), ncol, lchnk )
#else 
#ifdef SPMOMTRANS
   call outfld('UCONVMOM', ptend%u(1:ncol,:), ncol, lchnk )
   call outfld('VCONVMOM', ptend%v(1:ncol,:), ncol, lchnk )
   ...
   !       is that consistent with crm_output here? (should verify)
   call outfld('CLOUD   ',crm_output%cld(1:ncol,:), ncol, lchnk )

   ! vertically integrated and time/space averaged crm clouds:
   call outfld('CLDTOT  ',crm_output%cltot(1:ncol), ncol, lchnk )
   call outfld('CLDHGH  ',crm_output%clhgh(1:ncol), ncol, lchnk )
   ...
   ! because in crm_module this is set over k_crm, not k_gcm!
   do k = 1, crm_nz
      kgcm = pver - k + 1
      cldtop_gcm(1:ncol, kgcm) = crm_output%cldtop(1:ncol, k)
   end do
   ...
   if ( phys_stage == 1 ) then 
      call outfld('SPDT1   ', crm_output%spdt  (1:ncol,:),   ncol, lchnk )
      call outfld('SPDQ1   ', ptend%q          (1:ncol,:,1), ncol, lchnk )
      call outfld('SPQPEVP1', crm_output%qp_evp(1:ncol,:),   ncol, lchnk )
      call outfld('SPTLS1  ', crm_output%t_ls  (1:ncol,:),   ncol, lchnk )
   else
      call outfld('SPDT2   ', crm_output%spdt  (1:ncol,:),   ncol, lchnk )
      call outfld('SPDQ2   ', ptend%q          (1:ncol,:,1), ncol, lchnk )
      call outfld('SPQPEVP2', crm_output%qp_evp(1:ncol,:),   ncol, lchnk )
      call outfld('SPTLS2  ', crm_output%t_ls  (1:ncol,:),   ncol, lchnk )
   end if
#endif
   ...
   call outfld('EU_CRM  ', crm_output%eu_crm(1:ncol,:), ncol, lchnk)
   call outfld('DU_CRM  ', crm_output%du_crm(1:ncol,:), ncol, lchnk)
   call outfld('ED_CRM  ', crm_output%ed_crm(1:ncol,:), ncol, lchnk)
...
subroutine crm_clubb_out(lchnk, clubb_buffer, &
   ...
   real(r8), dimension(pcols,crm_nx, crm_ny, crm_nz+1) :: crm_cld    
   real(r8), dimension(pcols,crm_nx, crm_ny, crm_nz)   :: clubb_tk   
   real(r8), dimension(pcols,crm_nx, crm_ny, crm_nz)   :: clubb_tkh  
   real(r8), dimension(pcols,crm_nx, crm_ny, crm_nz)   :: relvar     
   real(r8), dimension(pcols,crm_nx, crm_ny, crm_nz)   :: accre_enhan
   real(r8), dimension(pcols,crm_nx, crm_ny, crm_nz)   :: qclvar     

   ! clubb_buffer outputs
   call outfld('UP2     ', clubb_buffer(1:ncol,:,:,:,1) , ncol, lchnk )
   call outfld('VP2     ', clubb_buffer(1:ncol,:,:,:,2) , ncol, lchnk )
   call outfld('WPRTP   ', clubb_buffer(1:ncol,:,:,:,3) , ncol, lchnk )
   call outfld('WPTHLP  ', clubb_buffer(1:ncol,:,:,:,4) , ncol, lchnk )
   call outfld('WP2     ', clubb_buffer(1:ncol,:,:,:,5) , ncol, lchnk )
   call outfld('WP3     ', clubb_buffer(1:ncol,:,:,:,6) , ncol, lchnk )
   call outfld('RTP2    ', clubb_buffer(1:ncol,:,:,:,7) , ncol, lchnk )
   call outfld('THLP2   ', clubb_buffer(1:ncol,:,:,:,8) , ncol, lchnk )
   call outfld('RTPTHLP ', clubb_buffer(1:ncol,:,:,:,9) , ncol, lchnk )
   call outfld('UPWP    ', clubb_buffer(1:ncol,:,:,:,10), ncol, lchnk )
   call outfld('VPWP    ', clubb_buffer(1:ncol,:,:,:,11), ncol, lchnk )
   call outfld('CRM_CLD ', clubb_buffer(1:ncol,:,:,:,12), ncol, lchnk )

   ! additional clubb-related diagnostics
   call outfld('CLUBB_TK '  , clubb_tk   (1:ncol,:,:,:), ncol, lchnk )
   call outfld('CLUBB_TKH'  , clubb_tkh  (1:ncol,:,:,:), ncol, lchnk )
   call outfld('RELVAR'     , relvar     (1:ncol,:,:,:), ncol, lchnk )
   call outfld('ACCRE_ENHAN', accre_enhan(1:ncol,:,:,:), ncol, lchnk )
   call outfld('QCLVAR'     , qclvar     (1:ncol,:,:,:), ncol, lchnk )
...
#if defined( ECPP )
subroutine crm_ecpp_out( lchnk, crm_ecpp )
   !------------------------------------------------------------------------------------------ 
   ! Purpose: write ECPP output to history files
   !------------------------------------------------------------------------------------------ 
   integer,             intent(in) :: lchnk
   type(crm_ecpp_type), intent(in) :: crm_ecpp

   call outfld('ACEN    '     , crm_ecpp%acen            (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('ABND    '     , crm_ecpp%abnd            (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('ACEN_TF '     , crm_ecpp%acen_tf         (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('ABND_TF '     , crm_ecpp%abnd_tf         (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('MASFBND '     , crm_ecpp%massflxbnd      (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('RHCEN   '     , crm_ecpp%rhcen           (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('QCCEN   '     , crm_ecpp%qcloudcen       (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('QICEN   '     , crm_ecpp%qicecen         (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('QSINK_AFCEN'  , crm_ecpp%qlsink_afcen    (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('PRECRCEN'     , crm_ecpp%precrcen        (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('PRECSCEN'     , crm_ecpp%precsolidcen    (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('QSINK_BFCEN'  , crm_ecpp%qlsink_bfcen    (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('QSINK_AVGCEN' , crm_ecpp%qlsink_avgcen   (1:ncol,:,:,:,:), ncol, lchnk )
   call outfld('PRAINCEN'     , crm_ecpp%praincen        (1:ncol,:,:,:,:), ncol, lchnk )

   call outfld('WUPTHRES'     , crm_ecpp%wupthresh_bnd   (1:ncol,:), ncol, lchnk )
   call outfld('WDNTHRES'     , crm_ecpp%wdownthresh_bnd (1:ncol,:), ncol, lchnk )
   call outfld('WWQUI_CEN'    , crm_ecpp%wwqui_cen       (1:ncol,:), ncol, lchnk )
   call outfld('WWQUI_CLD_CEN', crm_ecpp%wwqui_cloudy_cen(1:ncol,:), ncol, lchnk )
   call outfld('WWQUI_BND'    , crm_ecpp%wwqui_bnd       (1:ncol,:), ncol, lchnk )
   call outfld('WWQUI_CLD_BND', crm_ecpp%wwqui_cloudy_bnd(1:ncol,:), ncol, lchnk )   

end subroutine crm_ecpp_out
#endif /* ECPP */
