...
subroutine phys_hyperviscosity_Tq( elem, hvcoord, hybrid, deriv, nt, nets, nete, dt_in )
   ...
   !----------------------------------------------
   ! Initial boundary communication
   ! (to make tendency fields continuous)
   !----------------------------------------------
   !!! prepare edge buffer - no weighting needed here
   do ie = nets, nete
   ...
   !!! unpack edge buffer - no inverse weighting needed
   do ie = nets, nete
   ...
   do ic = 1, ( hypervis_subcycle * factor_subcycle )
      ...
      !------------------------------------
#if defined( SP_DUMMY_HYPERVIS_Q )
      ! do ie = nets, nete
      !    !!! apply mass matrix
         ! do k = 1, nlev
      ...
      enddo ! ie = nets, nete
         ...
         ! enddo ! k
      ! enddo ! ie = nets, nete
#endif /* SP_DUMMY_HYPERVIS_Q */

#if defined( SP_DUMMY_HYPERVIS_T )
      do ie = nets, nete
         !!! apply mass matrix
         do k = 1, nlev
            elem(ie)%T_tend(:,:,k) = elem(ie)%T_tend(:,:,k) * elem(ie)%spheremp(:,:) 
         enddo ! k
         !!! prepare edge buffer
         call edgeVpack_nlyr(edge_g, elem(ie)%desc, elem(ie)%T_tend(:,:,1:nlev), nlev, nlev*0, nlev )
      enddo ! ie = nets, nete
      !!! boundary exchange
      call bndry_exchangeV(hybrid,edge_g)
      do ie = nets, nete
         !!! unpack edge buffer
         call edgeVunpack_nlyr(edge_g, elem(ie)%desc, elem(ie)%T_tend(:,:,1:nlev), nlev, nlev*0, nlev )
         !!! apply inverse mass matrix
         do k = 1, nlev
            elem(ie)%T_tend(:,:,k) = elem(ie)%T_tend(:,:,k) * elem(ie)%rspheremp(:,:)
         enddo ! k
      enddo ! ie = nets, nete
#endif /* SP_DUMMY_HYPERVIS_T */
      !------------------------------------
      ! DUMMY TESTING VERSION - NO HYPERVIS
      !------------------------------------
#endif /* SP_DUMMY_HYPERVIS */
