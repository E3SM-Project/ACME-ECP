...
!---------------------------------------------------------------------------------
  ...
  public :: cldefr, cldems, cldovrlap, cldclw, reitab, reltab, &
            cldefr_icol, reitab_icol, reltab_icol
...
!===============================================================================
  subroutine cldefr_icol(lchnk   , &
       landfrac,t       ,rel     ,rei     ,ps      ,pmid    , landm, icefrac, snowh)
!----------------------------------------------------------------------- 
! 
! Purpose: 
! Compute cloud water and ice particle size - for single physics column
! 
! Method: 
! use empirical formulas to construct effective radii
! 
...
!
    ...
    real(r8), intent(out) :: rei          ! Ice effective drop size (microns)
!
! following Kiehl
         call reltab_icol( t, landfrac, landm, icefrac, rel, snowh)

! following Kristjansson and Mitchell
         call reitab_icol( t, rei)

    return
  end subroutine cldefr_icol
...
!===============================================================================
  subroutine reltab_icol(t, landfrac, landm, icefrac, rel, snowh)
!----------------------------------------------------------------------- 
! 
! Purpose: 
! Compute cloud water size
...
! jrm Reworked effective radius algorithm
    ! Start with temperature-dependent value appropriate for continental air
    ! Note: findmcnew has a pressure dependence here
    rel = rliqland + (rliqocean-rliqland) * min(1.0_r8,max(0.0_r8,(tmelt-t)*0.05_r8))
    ! Modify for snow depth over land
    rel = rel + (rliqocean-rel) * min(1.0_r8,max(0.0_r8,snowh*10._r8))
    ! Ramp between polluted value over land to clean value over ocean.
    rel = rel + (rliqocean-rel) * min(1.0_r8,max(0.0_r8,1.0_r8-landm))
    ! Ramp between the resultant value and a sea ice value in the presence of ice.
    rel = rel + (rliqice  -rel) * min(1.0_r8,max(0.0_r8,icefrac))
! end jrm
  end subroutine reltab_icol

!===============================================================================
  subroutine reitab_icol(t, re)
    ...
    index = int(t-min_retab)
    index = min(max(index,1),len_retab-1)
    corr = t - int(t)
    re = retab(index)*(1._r8-corr)   &
         +retab(index+1)*corr
    !           re(i,k) = amax1(amin1(re(i,k),30.),10.)
    return
  end subroutine reitab_icol
!===============================================================================
