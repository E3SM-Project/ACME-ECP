...
! Solution? I removed all of the spaces in the addfld calls, add_default, and outfld calls.
   ...
   if (lisccp_sim) then
      ...
      call addfld('CLDPTOP_ISCCP',(/'cosp_scol'/),'I','Pa', &
                  'Cloud Top Pressure in each Subcolumn',flag_xyfill=.true.,&
                  fill_value=R_UNDEF)
   ...
   if (llidar_sim) then
      ...
      call addfld('MOL532_CAL',(/'lev'/),'A','m-1sr-1', &
                  'Lidar Molecular Backscatter (532 nm) ',flag_xyfill=.true., &
                  fill_value=R_UNDEF)
      ...
      call addfld('CLD_CAL_LIQ', (/'cosp_ht'/), 'A','percent', &
                  'Lidar Liquid Cloud Fraction', flag_xyfill=.true., &
                  fill_value=R_UNDEF)
      ! lclcalipsoice (time,alt40,loc)
      call addfld('CLD_CAL_ICE', (/'cosp_ht'/), 'A','percent', &
                  'Lidar Ice Cloud Fraction', flag_xyfill=.true., fill_value=R_UNDEF)
      ! lclcalipsoun (time,alt40,loc)
      call addfld('CLD_CAL_UN', (/'cosp_ht'/),'A','percent', &
                  'Lidar Undefined-Phase Cloud Fraction', flag_xyfill=.true., &
                  fill_value=R_UNDEF)
      ! lclcalipsotmp (time,alt40,loc)
      call addfld('CLD_CAL_TMP', (/'cosp_ht'/), 'A','percent', &
                  'NOT SURE WHAT THIS IS Cloud Fraction', flag_xyfill=.true.,&
                  fill_value=R_UNDEF)
   ...
   if (lmodis_sim) then
      ...
      call addfld('TAUTMODIS',horiz_only,'A','1', &
                  'MODIS Total Cloud Optical Thickness*CLTMODIS',flag_xyfill=.true., &
                  fill_value=R_UNDEF)
      ! float tauwmodis ( time, loc )
      call addfld('TAUWMODIS',horiz_only,'A','1', &
                  'MODIS Liquid Cloud Optical Thickness*CLWMODIS',flag_xyfill=.true.,&
                  fill_value=R_UNDEF)
      ! float tauimodis ( time, loc )
      call addfld('TAUIMODIS',horiz_only,'A','1', &
                  'MODIS Ice Cloud Optical Thickness*CLIMODIS',flag_xyfill=.true.,&
                  fill_value=R_UNDEF)
      ...
      call addfld('REFFCLWMODIS',horiz_only,'A','m', &
                  'MODIS Liquid Cloud Particle Size*CLWMODIS',flag_xyfill=.true.,&
                  fill_value=R_UNDEF)
      ! float reffclimodis ( time, loc )
      call addfld('REFFCLIMODIS',horiz_only,'A','m', &
                  'MODIS Ice Cloud Particle Size*CLIMODIS',flag_xyfill=.true., &
                  fill_value=R_UNDEF)
      ...
      call addfld('LWPMODIS',horiz_only,'A','kg m-2', &
                  'MODIS Cloud Liquid Water Path*CLWMODIS',flag_xyfill=.true.,&
                  fill_value=R_UNDEF)
      ...
      call addfld('CLMODIS',(/'cosp_tau_modis','cosp_prs      '/),'A','%', &
                  'MODIS Cloud Area Fraction',flag_xyfill=.true., &
                  fill_value=R_UNDEF)
...
! Arguments
   ...
   !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ! TODO: condense these 3 calls in preparation for adding support for MMF
   ! configurations. We should not be copy and pasting this code three times
   ! here...instead, maybe run some logic to determine which columns to run COSP
   ! on based on the sunlit criteria and then based on the A-Train sampling
   ! criteria. Then compress arrays and setup COSP objects to only run over
   ! those columns. With this, we would only need two separate blocks of COSP
   ! calls: one for the passive simulators (no A-Train sampling) and one for the
   ...
   if (cosp_runall) then
      ...
      !call cospsimulator_intr_out(ncol, lchnk, sgx, &
      !                            sgradar, sglidar, &
      !                            isccp, misr, modis, &
      !                            stradar, stlidar)
   ...
   if (.not.cosp_runall) then
      ...
      ! Only run the isccp, misr, and modis simulators
      ...
      ! if we are doing A-Train sampling, we need to run the radar and lidar
      ! simulators separately, so turn them off for this pass
      !if (cosp_sample_atrain) then
      ! Don't run the radar/lidar simulators by setting COSP input structure cfg
      ! These are run separately to handle a-train sampling
      ...
      if (Nday .gt. 0) then  !! only run COSP if there are daylight points
         ...
         ! NOTE: this is not really necessary, COSP will fill nighttime columns
         ! with R_UNDEF, but it probably does save some overhead by reducing the
         ! size of the internal COSP arrays
   ...
   !if (.false.) then
      ...
      else  ! all columns used; no sub-sampling
      ...
      ! if not atrain sampling and there are points that fit latlon criteria or 
      ! there are points to sample within atrain
      ...
      if (Natrain > 0) then
         ...
         call construct_cosp_gridbox(time, &                          ! 1 double precision = real(r8) X
            ...
            gbx%mr_hydro(:,:,I_CVCICE) = mr_ccice_atrain(1:Npoints,pver:1:-1)    ! mixing_ratio_convective_cloud_ice (kg/kg)
            ...
            if (lradar_sim) then
               if (Natrain .lt. ncol) then  
                  ...
                  ! (3d variables from radar simulator), loop over subcol and height to do expansion.
                  do isc=1,nscol_cosp
                     do ihml=1,nhtml_cosp
                        tmp(1:Natrain) = sgradar%Ze_tot(1:Natrain,isc,ihml)
                        call ExpDayNite(tmp,   Natrain, IdxAtrain, Nno, IdxNo, 1, pcols)       
                        dbze94(1:ncol,isc,ihml)=tmp(1:ncol)
                        tmp(1:pcols)=R_UNDEF   
                     end do
                  end do
                  do id=1,ndbze_cosp
                     do ih=1,nht_cosp
                        ...
                        call ExpDayNite(tmp,   Natrain, IdxAtrain, Nno, IdxNo, 1, pcols)       
                        cfad_dbze94(1:ncol,id,ih)=tmp(1:ncol)
                        tmp(1:pcols)=R_UNDEF                   
                     end do
                  end do
            ...
            if (llidar_sim) then
               if (Natrain < ncol) then 
                  ...
                  ! (3d variables)
                  atb532(1:ncol,1:nscol_cosp,1:nhtml_cosp) = sglidar%beta_tot  ! (time,height_mlev,column,profile)
                  cfad_lidarsr532(1:ncol,1:nsr_cosp,1:nht_cosp)= stlidar%cfad_sr  ! (time,height,scat_ratio,profile)
               end if
            end if

            if ((lfrac_out) .and. (.not.lradar_sim) .and. (.not.llidar_sim)) then
               frac_out(1:ncol,1:nscol_cosp,1:nhtml_cosp) = sgx%frac_out  ! time,height_mlev,column,profile
            end if
